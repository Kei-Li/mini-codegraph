...
const CACHE_ANNOTATIONS = ['Cacheable', 'CacheEvict', 'CachePut', 'Caching'] as const
...
export function indexCacheAnnotations(
  ...
  for (const node of allNodes) {
    ...
    for (const ann of anns) {
      if (CACHE_ANNOTATIONS.includes(ann.annotationName as any)) {
  ...
  for (const ann of annotations) {
    for (const cacheName of ann.cacheNames) {
      const allNodes2 = queries.getAllNodes()
      for (const otherNode of allNodes2) {
        if (otherNode.id === ann.nodeId) continue
        const otherAnns = queries.getAnnotationsByNode(otherNode.id)
        const match = otherAnns.some(a =>
          CACHE_ANNOTATIONS.includes(a.annotationName as any) &&
          a.value.includes(cacheName)
        )
        if (match) {
          queries.insertEdge(ann.nodeId, otherNode.id, 'cache_related',
