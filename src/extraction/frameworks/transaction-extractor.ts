...
export function parseTransactionalValue(value: string): Partial<TransactionalInfo> {
  ...
  for (const part of parts) {
    ...
    if (trimmed === 'readOnly = true' || trimmed === 'readOnly=true') {
      ...
      if (match) {
        const level = match[2]
        if (PROPAGATION_LEVELS.includes(level)) {
...
export function indexTransactionalAnnotations(
  ...
  moduleId: string,
  nodeAnnCache?: Map<string, { annotationName: string; value: string }[]>,
): TransactionalInfo[] {
...
export function findTxBoundaryConflicts(
  queries: QueryManager,
  _moduleId: string,
  annCache?: Map<string, { annotationName: string; value: string }[]>,
): {
    ...
    try {
      ...
      if (outerNode && innerNode) {
        const outerOuter = queries.getCallers(edge.sourceId)
          ...
          .filter(Boolean)
          .map(n => {
            const aa = annCache?.get(n!.id) ?? queries.getAnnotationsByNode(n!.id)
