...
export function detectRoutes(
  ...
  if (existsSync(pkgJsonPath)) {
    try {
      ...
      if (deps.express) routes.push(...detectExpressRoutes(projectRoot))
      if (deps['@nestjs/core']) routes.push(...detectNestRoutes(projectRoot))
      if (deps['next']) routes.push(...detectNextRoutes(projectRoot))
      if (deps.react || deps['react-router-dom']) routes.push(...detectReactRouterRoutes(projectRoot))
      if (deps.fastify) routes.push(...detectFastifyRoutes(projectRoot))
      if (deps.koa) routes.push(...detectKoaRoutes(projectRoot))
      if (deps.nuxt || deps['nuxt3'] || deps['@nuxt/kit']) routes.push(...detectNuxtRoutes(projectRoot))
    } catch {
      // ignore parse errors
    }
  }
  ...
  if (existsSync(pomPath) || existsSync(buildGradlePath)) {
    routes.push(...detectSpringRoutes(projectRoot))
  ...
  if (existsSync(requirementsTxt)) {
    try {
      ...
      if (/^Django/im.test(pip)) routes.push(...detectDjangoRoutes(projectRoot))
      if (/^Flask/im.test(pip)) routes.push(...detectFlaskRoutes(projectRoot))
    } catch { /* silent */ }
  }
  ...
  if (existsSync(gemfile)) {
    try {
      ...
      if (/gem\s+['"]rails['"]/im.test(gem)) routes.push(...detectRailsRoutes(projectRoot))
    } catch { /* silent */ }
  }
  ...
  if (existsSync(goMod)) {
    try {
      ...
      if (/gin-gonic\/gin/.test(mod)) routes.push(...detectGinRoutes(projectRoot))
    } catch { /* silent */ }
  }
  ...
  if (existsSync(composerJson)) {
    try {
      ...
      if (deps?.['laravel/framework']) routes.push(...detectLaravelRoutes(projectRoot))
    } catch { /* silent */ }
  }
  ...
  for (const csproj of csprojFiles) {
    try {
      ...
      if (/Microsoft\.AspNetCore/.test(csprojContent)) {
        routes.push(...detectAspNetCoreRoutes(projectRoot))
        break
      }
    } catch { /* silent */ }
  }
...
function detectExpressRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectNestRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectNextRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectReactRouterRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectFastifyRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectKoaRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
  const files = findFilesByApiPattern(projectRoot, ['.js', '.ts', '.mjs'])
...
function detectSpringRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectDjangoRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectFlaskRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectRailsRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectGinRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectLaravelRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectAspNetCoreRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
...
function detectNuxtRoutes(projectRoot: string): RouteInfo[] {
  const routes: RouteInfo[] = []
