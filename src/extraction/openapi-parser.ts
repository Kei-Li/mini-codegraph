      ...
      for (const [path, methods] of Object.entries(paths)) {
        for (const [method, detail] of Object.entries(methods as any)) {
          ...
          endpoints.push({
            ...
            operationId: d.operationId ?? `${method}_${path.replace(/[/{}]/g, '_')}`,
            serviceName: d['x-service'] ?? d.tags?.[0],
            parameters: (d.parameters ?? []).map((p: any) => ({
              name: p.name, in: p.in, required: p.required ?? false,
            })),
            requestBody: d.requestBody?.content?.['application/json']?.schema?.$ref,
            responses: Object.fromEntries(
              Object.entries(d.responses ?? {}).map(([code, r]: [string, any]) => [code, r.description ?? ''])
            ),
          })
        }
      }
