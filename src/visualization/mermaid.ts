...
export function buildFanoutMermaid(
  fanoutTrace: import('../types.js').PageFanoutTrace,
  maxBranches = 8,
): string {
  const lines: string[] = ['graph TB']
  const pageId = `page_${escapeId(fanoutTrace.pageFile)}`
  const pageLabel = fanoutTrace.pageFile.split('/').pop() ?? fanoutTrace.pageFile
  lines.push(`  ${pageId}["${pageLabel}"]`)
  lines.push(`  style ${pageId} fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px`)

  let branchCount = 0
  for (const branch of fanoutTrace.branches) {
    if (branchCount >= maxBranches) {
      lines.push(`  more["... +${fanoutTrace.branches.length - maxBranches} more APIs"]`)
      lines.push(`  ${pageId} -.-> more`)
      break
    }
    const apiId = `api_${escapeId(branch.method)}_${escapeId(branch.path)}_${branchCount}`
    lines.push(`  ${apiId}["${branch.method} ${branch.path}"]`)
    lines.push(`  style ${apiId} fill:#fff3e0,stroke:#e65100`)
    lines.push(`  ${pageId} --> ${apiId}`)

    let prevId = apiId
    for (let hi = 0; hi < branch.trace.length && hi < 15; hi++) {
      const hop = branch.trace[hi]
      const hopId = `hop_${branchCount}_${hi}`
      const label = `${hop.kind === 'controller_endpoint' ? '🎯 ' : ''}${hop.name}`
      lines.push(`  ${hopId}["${label}"]`)
      const hopStyles: Record<string, string> = {
        controller_endpoint: 'fill:#e1f5fe,stroke:#0288d1',
        service_method: 'fill:#e8f5e9,stroke:#388e3c',
        mybatis_mapper: 'fill:#fce4ec,stroke:#c62828',
        feign_call: 'fill:#fff9c4,stroke:#f9a825',
        vue_api_call: 'fill:#f3e5f5,stroke:#7b1fa2',
      }
      const style = hopStyles[hop.kind]
      if (style) lines.push(`  style ${hopId} ${style}`)
      lines.push(`  ${prevId} --> ${hopId}`)
      prevId = hopId
    }
    branchCount++
  }

  if (fanoutTrace.involvedServices.length > 0) {
    lines.push(`  services["Services: ${fanoutTrace.involvedServices.join(', ')}"]`)
    lines.push(`  style services fill:#f5f5f5,stroke:#9e9e9e,stroke-dasharray: 5 5`)
  }
