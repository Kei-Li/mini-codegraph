...
export function createCallersHandler(): ToolDefinition {
  return {
    name: 'mini_cg_callers',
    description: 'Find all functions/methods that call a specific symbol. Merges local and external callers.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (args, graph) => {
      ...
      if (results.length === 0) return { symbol, callers: [], total: 0, truncated: false }

      const allCallers = graph.getCallersWithExternal(results[0].node.id)
      const safeLimit = Math.min(Math.max(1, rawLimit ?? 50), 500)
      const p = {
        items: allCallers.slice(offset, offset + safeLimit),
        total: allCallers.length,
        truncated: allCallers.length > offset + safeLimit,
      }
      return {
        symbol,
