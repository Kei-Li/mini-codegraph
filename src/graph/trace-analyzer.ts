            ...
            for (const jpaEdge of jpaEdges) {
              ...
              if (jpaNode) {
                trace.push({
                  ...
                  detail: `Table: ${(JSON.parse(jpaNode.signature ?? '{}') as Record<string, unknown>).table ?? jpaNode.name}`,
                })
              }
            }
          }
...
export function indexTraces(queries: QueryManager, _moduleId: string): FullTrace[] {
  const traces = buildFullTraces(queries)
